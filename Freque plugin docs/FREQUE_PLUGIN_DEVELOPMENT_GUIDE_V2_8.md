# Freque Plugin Development Guide v2.8
...
- **Enhanced Audio Features** - Complete update with all latest audio detection improvements ⭐ NEW
- **7-Band Frequency System** - Access to subBass, bass, lowMid, mid, highMid, treble, and air frequency bands ⭐ NEW
- **Beat Strength & Confidence** - New beat detection metrics (beatStrength, beatConfidence) for more nuanced beat reactivity ⭐ NEW
- **Tempo Confidence & Candidates** - Access to tempo confidence scores and alternative tempo suggestions ⭐ NEW
- **Spectral Centroid (Dominant Frequency)** - Weighted average frequency with temporal smoothing for stable frequency detection ⭐ NEW
- **Harmonic Features** - Chord, key, and progression detection now available to all plugins (independent of Autopilot) ⭐ NEW
- **Enhanced Beat Detection** - Frequency-weighted, bass-focused beat detection with multi-factor analysis ⭐ NEW
- **Professional Tempo Detection** - Histogram-based tempo detection with autocorrelation, similar to Ableton Live ⭐ NEW

## What's New in v2.7

- **Credits Section** - Display plugin credits in mixer channel strips (max 140 chars, truncated with ellipsis if longer) ⭐ NEW

## What's New in v2.6

- **Critical NaN Prevention** - Always use `|| 0` fallback for sharedAudioData to prevent black screens ⭐ NEW
- **WebGL Shader Debugging** - Complete troubleshooting guide for shader compilation and uniform issues ⭐ NEW

## What's New in v2.5

- **Window Resize Handling** - Complete guide to responsive canvas sizing and game state preservation
- **Audio Integration Clarification** - Clear documentation on how to properly access audio data
...
3. [Window Resize Handling](#window-resize-handling) ⭐ NEW
4. [2D Canvas Plugins](#2d-canvas-plugins)
5. [WebGL/WebGL2 Plugins](#webglwebgl2-plugins) ⭐ NEW
6. [Three.js Integration](#threejs-integration)
7. [Advanced Three.js Techniques](#advanced-threejs-techniques)
8. [Audio Integration](#audio-integration)
9. [Advanced Audio Reactivity](#advanced-audio-reactivity) ⭐ NEW
10. [Ring Spawning System](#ring-spawning-system) ⭐ NEW
11. [Post-Processing Effects](#post-processing-effects) ⭐ NEW
...
class MyPlugin extends FrequePluginBase {
    constructor(visualizer) {
        super('myplugin', visualizer, {
            ...
            credits: 'Plugin by Your Name. Based on original concept by...' // Optional: Credits text (max 140 chars recommended)
        });
...
**Why Throttling?**
- **Without throttle**: Plugins resize on every pixel change → **visible flickering** during window drag
- **With throttle**: Plugins resize once after drag stops → **smooth experience**
- **150ms delay**: Optimal balance between responsiveness and flicker prevention
...
**Why Percentage CSS?**
- ✅ Canvas display size scales with container automatically
- ✅ Works correctly with window resize without manual CSS updates
- ✅ Maintains aspect ratio properly
- ❌ Fixed pixel CSS (`width: 800px`) doesn't scale with window
...
**What This Achieves:**
1. **Ship** → Repositioned to center of new canvas
2. **Barriers** → Recreated at correct positions for new width
3. **Aliens** → Recreated in proper grid for new canvas size (game state preserved)
4. **Stars** → Regenerated to fill new canvas dimensions
5. **Score/Wave/Lives** → Preserved throughout resize
...
- [ ] Test that game state persists through resize
...
## Understanding Audio Data Access (v2.8)
...
onUpdate(deltaTime, timestamp, sharedAudioData) {
    ...
    // ✅ 7-Band Frequency System - use sharedAudioData.frequencyBands
    // CRITICAL: Always use || 0 fallback to prevent NaN!
    this.subBassLevel = sharedAudioData.frequencyBands?.subBass || 0;
    this.bassLevel = sharedAudioData.frequencyBands?.bass || 0;
    this.lowMidLevel = sharedAudioData.frequencyBands?.lowMid || 0;
    this.midLevel = sharedAudioData.frequencyBands?.mid || 0;
    this.highMidLevel = sharedAudioData.frequencyBands?.highMid || 0;
    this.trebleLevel = sharedAudioData.frequencyBands?.treble || 0;
    this.airLevel = sharedAudioData.frequencyBands?.air || 0;
    
    // ✅ Enhanced Beat Detection
    this.beatDetected = sharedAudioData.beat || false;
    this.beatStrength = sharedAudioData.beatStrength || 0;
    this.beatConfidence = sharedAudioData.beatConfidence || 0;
    
    // ✅ Tempo with Confidence
    this.tempo = sharedAudioData.tempo || 120;
    this.tempoConfidence = sharedAudioData.tempoConfidence || 0;
    this.tempoCandidates = sharedAudioData.tempoCandidates || [];
    
    // ✅ Dominant Frequency (Spectral Centroid)
    this.dominantFreq = sharedAudioData.dominantFrequency || 0;
    
    // ✅ Harmonic Features (Chords, Keys)
    this.currentChord = sharedAudioData.harmonic?.chord || null;
    this.currentKey = sharedAudioData.harmonic?.key || null;
    this.chordConfidence = sharedAudioData.harmonic?.chordConfidence || 0;
    this.keyConfidence = sharedAudioData.harmonic?.keyConfidence || 0;
...
**⚠️ CRITICAL: Always Use `|| 0` Fallback**

If you don't use the `|| 0` fallback, `undefined` audio values will cause **NaN (Not a Number)** to propagate through your calculations, resulting in:
- Black/invisible rendering
- Broken animations
- Shader failures (NaN uniforms render as black)

**Why this happens:**
```javascript
// ❌ WITHOUT fallback - causes NaN
this.value = this.flashIntensity * (1 + sharedAudioData.bass);
// If sharedAudioData.bass is undefined: 0.4 * (1 + undefined) = 0.4 * NaN = NaN

// ✅ WITH fallback - safe
this.value = this.flashIntensity * (1 + (sharedAudioData.bass || 0));
// If sharedAudioData.bass is undefined: 0.4 * (1 + 0) = 0.4
```

This is especially critical for:
- WebGL/shader uniforms (NaN = black screen)
- Audio-reactive calculations
- Smoothing/interpolation math
...
**From sharedAudioData Parameter - Basic Features:**
- `sharedAudioData.energy` - Overall audio intensity (0.0-1.0)
- `sharedAudioData.beat` - Beat detected this frame (boolean)
- `sharedAudioData.beatStrength` - How strong the beat is (0.0-1.0, persists between beats)
- `sharedAudioData.beatConfidence` - Confidence in beat detection (0.0-1.0)
- `sharedAudioData.tempo` - Estimated tempo in BPM (smoothed, with persistence)
- `sharedAudioData.tempoConfidence` - Confidence in tempo detection (0.0-1.0, persistent)
- `sharedAudioData.tempoCandidates` - Alternative tempo suggestions `[{bpm: N, confidence: M}]`
- `sharedAudioData.dominantFrequency` - Spectral centroid in Hz (weighted average, temporally smoothed)
- `sharedAudioData.dominantFreq` - Dominant frequency band ('subBass'/'bass'/'lowMid'/'mid'/'highMid'/'treble'/'air')

**From sharedAudioData Parameter - 7-Band Frequency System:**
- `sharedAudioData.frequencyBands.subBass` - Sub-bass energy (20-60 Hz, 0.0-1.0)
- `sharedAudioData.frequencyBands.bass` - Bass energy (60-250 Hz, 0.0-1.0)
- `sharedAudioData.frequencyBands.lowMid` - Low-mid energy (250-500 Hz, 0.0-1.0)
- `sharedAudioData.frequencyBands.mid` - Mid energy (500-2000 Hz, 0.0-1.0)
- `sharedAudioData.frequencyBands.highMid` - High-mid energy (2000-4000 Hz, 0.0-1.0)
- `sharedAudioData.frequencyBands.treble` - Treble energy (4000-8000 Hz, 0.0-1.0)
- `sharedAudioData.frequencyBands.air` - Air energy (8000-22050 Hz, 0.0-1.0)

**Legacy Shortcuts (for backwards compatibility):**
- `sharedAudioData.bass` - Same as `sharedAudioData.frequencyBands.bass`
- `sharedAudioData.mid` - Same as `sharedAudioData.frequencyBands.mid`
- `sharedAudioData.treble` - Same as `sharedAudioData.frequencyBands.treble`

**From sharedAudioData Parameter - Advanced Features:**
- `sharedAudioData.energyChange` - Change in energy from previous frame (0.0-1.0)
- `sharedAudioData.flux` or `sharedAudioData.spectralFlux` - Rate of frequency change (0.0-1.0)
- `sharedAudioData.morphIntensity` - Combined "interestingness" metric (0.0-1.0)

**From sharedAudioData Parameter - Harmonic Features (Independent of Autopilot):**
- `sharedAudioData.harmonic.chord` - Detected chord (e.g., 'C major', 'Am', 'diminished')
- `sharedAudioData.harmonic.chordConfidence` - Chord detection confidence (0.0-1.0)
- `sharedAudioData.harmonic.key` - Detected musical key (e.g., 'C', 'G', 'Dm')
- `sharedAudioData.harmonic.keyConfidence` - Key detection confidence (0.0-1.0)
- `sharedAudioData.harmonic.progression` - Recent chord progression history (Array)

**From sharedAudioData Parameter - Raw Data:**
- `sharedAudioData.frequencies` - Raw frequency data array (Uint8Array, 4096 values)
- `sharedAudioData.dataArray` - Same as frequencies (for compatibility)
...
Plugins can display credits text in the mixer channel strip:
...
super('myplugin', visualizer, {
    ...
    author: 'Your Name',
    description: 'My Awesome Plugin',
    credits: 'Plugin by Your Name. Based on original concept by...' // Optional
});
```

**How It Works:**
- Set `credits` in plugin config (constructor's second parameter)
- Credits text is displayed in a "Credits" section below the Controls section in the mixer
- **140 character limit recommended** - text will be truncated with ellipsis (`...`) if longer
- Credits section is automatically hidden if no credits text is provided
- Simple static text display (not collapsible like Presets/Controls)

**Best Practices:**
- Keep credits concise (aim for 140 characters or less)
- Include author name and any attribution to original concepts/sources
- Example: `'Audio-reactive visualization by Your Name. Based on feedback-shader.html.'`
...
class MyPlugin extends FrequePluginBase {
    constructor(visualizer) {
        super('myplugin', visualizer, {
            version: '1.0.0',
            author: 'Your Name',
            credits: 'Plugin by Your Name. Based on original concept by Original Author.' // 89 chars
        });
    }
}
```
...
Color morphing is the smooth transition between colors over time, creating flowing, organic color changes. Common uses:
- Cycling through a color palette
- Audio-reactive color shifts
- Smooth transitions between user-selected schemes
- Dynamic hue/saturation/brightness changes
...
Always give users control over:
- **Toggle** - Enable/disable morphing
- **Speed** - How fast colors change
