...
class NebulaPlugin extends FrequePluginBase {
    ...
    // Throttled to batch updates and prevent blocking during dial rotation
    updateNebulaSetting(property, value, nested = null) {
        // Store the pending update (will overwrite previous pending value for same property)
        const key = nested ? `${property}.${nested}` : property;
        this.pendingUpdates.set(key, { property, value, nested });
        
        // Schedule update on next animation frame (throttles rapid dial changes)
    ...
    flushPendingUpdates() {
        ...
        this.pendingUpdates.forEach((update) => {
            if (update.nested) {
                ...
                if (!this.nebulaViz.settings[update.property]) {
                    this.nebulaViz.settings[update.property] = {};
                }
                this.nebulaViz.settings[update.property][update.nested] = update.value;
            } else {
                // Use NebulaVisualization's updateSetting method for proper updates
                this.nebulaViz.updateSetting(update.property, update.value);
            }
        });
