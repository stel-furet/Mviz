# 🏷️ Rename to freque - Complete Inventory

**Goal**: Replace all references to `Vizzy`, `MVPro`, `GitItUp`, and `GitUp` with `freque` throughout the codebase.
...
| `GitItUp` | `Freque` |
| `GitUp` | `Freque` |
| `vizzy.css` | `freque.css` |
| `VizzyPlaylistDB` | `FrequePlaylistDB` |
| `Vizzy_Recording` | `Freque_Recording` |
| `GitItUp Music Visualizer` | `Freque Music Visualizer` |
| `GitItUp Live Display` | `Freque Live Display` |
| `MVPRO Mixer` | `FREQUE Mixer` |
...
- Consider migration strategy for:
  - `vizzy_current_playlist` → `freque_current_playlist`
  - `vizzy-theme` → `freque-theme`
  - `gitup_record_settings` → `freque_record_settings`
  - `gitup_display_settings` → `freque_display_settings`
  - `gitup_background_image` → `freque_background_image`
  - `gitup_bgcolor` → `freque_bgcolor`

### **BroadcastChannel Names**
- Changing channel names will **break live display communication**
- Ensure all instances are updated simultaneously:
  - `gitup-live-display` → `freque-live-display`
  - `mvpro-live-display-*` → `freque-live-display-*`
  - `mvpro-live-display-settings-*` → `freque-live-display-settings-*`

### **File Dependencies**
- `vizzy.css` → `freque.css` requires updating all HTML references
- `js/mvpro_webgl.js` → consider renaming to `js/freque_webgl.js`

### **Database Names**
- `VizzyPlaylistDB` → `FrequePlaylistDB` will require database migration
